module DomainTheory.ContinuousFunctions.cur-cont where
...
helpful-chain-1 {_} {D′} f c d′ = record
    { g = λ x i → pair d′ (g c x) i
    ; mon = λ a≤a′ → (λ {fzero → reflexive (pos D′); (fsucc fzero) → mon c a≤a′})
    }
...
helpful-chain-2 {D} f c d = record
    { g = λ x i → pair (g c x) d i
    ; mon = λ x → (λ { fzero → mon c x ; (fsucc fzero) → reflexive (pos D) })
    }
